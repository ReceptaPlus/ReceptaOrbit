...
export default async function ClienteFichaPage({
  ...
  return (
    <div className="space-y-6">
      <header>
        <Link href="/app/clientes" className="text-sm text-secondary hover:text-primary">
          ← Clientes
        </Link>
        ...
        <section className="bg-card rounded-xl border border-line p-5">
          ...
          {cycles.length === 0 ? (
            ...
            <ul className="divide-y divide-line-subtle">
              {cycles.map((c) => (
                <li key={c.id}>
                  <Link href={`/app/conversas/${c.id}`} className="flex items-center gap-3 py-3 hover:bg-subtle -mx-2 px-2 rounded-lg">
