...
const MAX_BAR = Math.max(...dashboard.weekBars);

export default function DashboardPage() {
  const recent = conversationCycles.slice(0, 5);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <KpiCard label="Vendas pendentes de revisão" value={String(dashboard.pendingReviewCount)} delta={`${dashboard.openConversations} conversas abertas`} neutral />
      </section>
      ...
      <section className="bg-card rounded-xl border border-line p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/app/conversas" className="text-sm text-primary hover:underline">Ver todas</Link>
        </div>
        <ul className="divide-y divide-line-subtle">
          {recent.map((c) => (
            <li key={c.id}>
              <Link href={`/app/conversas/${c.id}`} className="flex items-center gap-4 py-3 hover:bg-subtle -mx-2 px-2 rounded-lg">
